...
function InvoiceView({ currentJob, updateJob, rates, companySettings={} }) {
  ...
  const CS_NAME = cs.name || COMPANY;
  const CS_PHONE = cs.phone || PHONE;
  const CS_EMAIL = cs.email || EMAIL;
  const CS_LOGO = cs.logoB64 || LOGO_B64;
  const CS_LEGAL = cs.legalTerms || "";
  const CS_DEPOSIT = cs.depositTerms || "A 25% deposit is required to schedule work. Balance due upon completion.";
...
function ReportsView({ jobs, rates, setCurrentJob, setView, companySettings={} }) {
  ...
  const printReport = async () => {
    ...
    try {
      ...
      doc.save(((cs.name||"Company").replace(/[^a-zA-Z0-9]/g,"_").slice(0,20))+"_Profitability_Report_"+new Date().toISOString().slice(0,10)+".pdf");
    } catch(e) {
      console.error(e);
      alert("PDF generation failed: "+e.message);
    }
...
// ─── Owner Hub — landing screen linking to owner tools ───────────
function AdminHubView({ setView, setCurrentJob, iconStyle, syncIconStyle, companySettings={}, syncCompanySettings }) {
  const cards = [
    { key:"company-settings", icon:"🏢", title:"Company Settings", desc:"Update your company name, contact info, logo, and legal terms." },
    { key:"permissions", icon:"🔐", title:"Permissions", desc:"View what each role can see and edit. Contact BlacktopIQ admin to make changes." },
    ...
    { key:"referral",    icon:"🔗", title:"Referral Link", desc:"Share your unique referral link and earn rewards when contractors sign up." },
  ];
    ...
    <div style={S.page}>
      ...
      <section style={S.section}>
        ...
        <p style={{fontSize:11, color:C.textDim, marginTop:12}}>
          Owner always retains full access to everything regardless of these settings. Permissions are set by the platform admin.
        </p>
      </section>
    </div>
  );
...
function ExportView({ jobs, laborEntries, rates, setView, companySettings={} }) {
  ...
  const exportJobsCSV = () => {
    ...
    downloadCSV(((cs.name||"Company").replace(/[^a-zA-Z0-9]/g,"_").slice(0,20))+"_Jobs_Export_"+new Date().toISOString().slice(0,10)+".csv", rows);
  };
  ...
  const exportCostsCSV = () => {
    ...
    downloadCSV(((cs.name||"Company").replace(/[^a-zA-Z0-9]/g,"_").slice(0,20))+"_Costs_Export_"+new Date().toISOString().slice(0,10)+".csv", rows);
  };
  ...
  const exportLaborCSV = () => {
    ...
    downloadCSV(((cs.name||"Company").replace(/[^a-zA-Z0-9]/g,"_").slice(0,20))+"_Labor_Export_"+new Date().toISOString().slice(0,10)+".csv", rows);
  };
...
function JobsView({ jobs, setJobs, deleteJob, setCurrentJob, setView, rates, updateJobById, userRole, userId, crews }) {
  ...
  const visibleJobs = (() => {
    if (userRole === "owner" || userRole === "manager") return jobs;
  ...
  const create    = () => { const j=initialJob(rates); if (userId && userRole!=="owner" && userRole!=="manager") j.assignedTo = userId; setJobs(p=>[j,...p]); setCurrentJob(j); setView("jobdetail"); };
...
// ─── Estimate View ────────────────────────────────────────────────────────────
function EstimateView({ currentJob, updateJob, rates, syncJob, companySettings={} }) {
  const cs = companySettings;
  const CS_NAME = cs.name || COMPANY;
  const CS_PHONE = cs.phone || PHONE;
  const CS_EMAIL = cs.email || EMAIL;
  ...
  const emailEstimate = () => {
    if (!CS_LEGAL) {
      alert("⚠️ Please set your legal terms before sending estimates. Go to Owner Hub → Company Settings to add them.");
      return;
    }
...
function CompanySetupWizard({ accessToken, userId, onComplete, syncCompanySettings }) {
  ...
  const saveAndFinish = async () => {
    ...
    try {
      ...
      await sbFetch("company_settings", {
        method: "POST",
        headers: { "Prefer": "resolution=merge-duplicates,return=minimal" },
        body: JSON.stringify({ id: 1, data: csData }),
      }, accessToken);
      if (syncCompanySettings) syncCompanySettings(csData);
  ...
  const stepTitle = ["Your Profile", "Company Info", "Company Address", "Logo"][step - 1];
    ...
    <div style={{...S.app, alignItems:"center", justifyContent:"center"}}>
      <div style={{maxWidth:400, width:"100%", padding:"0 24px"}}>
        ...
        <div style={{display:"flex", gap:4, marginBottom:20}}>
          {[1,2,3,4].map(s => (
            <div key={s} style={{flex:1, height:4, borderRadius:2,
              background: s <= step ? C.accent : C.surface2}}/>
          ))}
        </div>
          ...
          <>
            <label style={S.formLabel}>Company Name *
              <input type="text" value={companyName} onChange={e => setCompanyName(e.target.value)}
                style={S.input} placeholder="Acme Asphalt Maintenance, LLC"/>
            </label>
            <label style={{...S.formLabel, marginTop:10}}>Company Phone
              <input type="tel" value={companyPhone} onChange={e => setCompanyPhone(e.target.value)}
                style={S.input} placeholder="(555) 555-5555"/>
            </label>
            <label style={{...S.formLabel, marginTop:10}}>Estimate Email
              <input type="email" value={companyEmail} onChange={e => setCompanyEmail(e.target.value)}
                style={S.input} placeholder="estimates@yourcompany.com" autoCapitalize="none"/>
            </label>
            <label style={{...S.formLabel, marginTop:10}}>Website
              <span style={{fontSize:11, color:C.textDim, fontWeight:400}}> (optional)</span>
              <input type="url" value={website} onChange={e => setWebsite(e.target.value)}
                style={S.input} placeholder="https://yourcompany.com"/>
            </label>
          </>
        )}
        ...
        {step === 3 && (
          <>
            ...
            <div style={{...S.formGrid, marginTop:10}}>
              ...
              <label style={S.formLabel}>State
                <input type="text" value={stateF} onChange={e => setStateF(e.target.value)}
                  style={S.input} placeholder="PA" maxLength={2}/>
              </label>
            </div>
            <label style={{...S.formLabel, marginTop:10}}>ZIP Code
              <input type="text" value={zip} onChange={e => setZip(e.target.value)}
                style={S.input} placeholder="18360" maxLength={10}/>
            </label>
          </>
        ...
        {step === 4 && (
          <>
            <p style={{fontSize:12, color:C.textMuted, marginBottom:12}}>
              Upload your company logo. It will appear on estimates and invoices.
              Recommended: PNG, under 500 KB.
            </p>
            ...
            {!logoName && <p style={{fontSize:12, color:C.textDim}}>
              No logo? You can add one later in Owner Hub → Company Settings.
            </p>}
          </>
        )}
        ...
        <div style={{display:"flex", gap:8, marginTop:18}}>
          {step > 1 && (
            <button style={{...S.btnSecondary, flex:1}} onClick={() => setStep(s => s - 1)}>
              ← Back
            </button>
          )}
        ...
        {step === 4 && (
          <button style={{...S.btnSecondary, width:"100%", marginTop:8, fontSize:12}} onClick={saveAndFinish}>
            Skip logo for now
          </button>
        )}
      </div>
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        try {
          const csr = await sbFetch("company_settings?select=data&limit=1");
          const csd = await csr.json();
          if (Array.isArray(csd) && csd.length > 0 && csd[0].data) {
            setCompanySettings(prev => ({...prev, ...csd[0].data}));
            // Company settings exist → no wizard needed (even if profile was incomplete)
